...
MaketreeProk_Function() {
    ...
    align_seqs.py -i $USER_PATH/$INFILE -m pynast -t $REP_ALIGNED_PATH -o $USER_PATH/aligned_seqs_$ELEMENT/ -p 0.30
...
# Run Midas workflow if selected
   if [ $WORKFLOW = "M" ]
     ...
     # Automatically define reference database to latest midas file.
     REFDATABASE=2
     ...
     # Write command parameters to log
     Run_Time_Params_Function ASVPIPELINE
